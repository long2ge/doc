...
### mongorestore 和 mongodump
mongorestore --host 127.0.0.1 --oplogRelay dump

分片集备份
分片集备份大致与复制集原理相同，不过存在以下差异：
1. 应分片为每个片和config备份
2. 分片集备份不仅要考虑一个分片内的一致性问题，还要考虑分片间的一致性问题，因此每个片要能够恢复到同一个时间点。

分片集的增量备份
1. 理论上我们可以使用与复制集相同的方式来为分片集完成增量备份，但实际上分片集的情况更加复杂。这些复杂性来自两个方面
2. 各个数据节点的时间不一致，每个数据节点很难完全恢复到一个真正的一致时间上，通常只能做到大致一致，而这种大致一致通常足够好，除了一些情况之外。
3. 分片之间的数据迁移。当一部分数据从一个片迁移到另外一个片时，
最终数据到底在哪里取决于config中的元数据。如果元数据与数据节点之间的时间差
正好导致数据实际已经迁移到新分片上，而元数据仍然认为数据在旧分片上，就会
导致数据丢失情况发生。虽然这种情况发生的概率很小，但仍有可能导致问题。
4. 要避免上述问题的发生，只有定期停止均衡器；只有在均衡器停止期间，增量
恢复才能保证正确。
...
网络和操作系统加固
1. 使用专用用户运行mongodb
1.1 不建议在操作系统层面使用root用户运行Mongodb
2. 限制网络开放度
2.1 通过防火墙，iptables规则控制对Mongodb的访问
2.2 使用VPN / VPCs 可以创建一个安全通道，Mongodb服务不应该直接暴露在互联网上
2.3 使用白名单列表限制允许访问的网段
2.4 使用bind_ip 绑定一个具体地址
2.5 修改默认监听端口27017
3. 使用安全配置选项运行Mongdb
3.1 如果不需要执行javascriopt脚本，使用--noscripting 禁止脚本执行
3.2 如果使用老版本Mongodb，关闭http接口： net.hhtp.enabled=false
net.http.JSONPEnabled=false
3.3 如果使用老版本Mongdb，关闭Rest API 接口 net.http.RESTinterfaceEnabled=false
...
Demo : 启动认证
1. 命令行方式通过 --auth 参数
2. 配置文件方法在security下添加authorization：enabled
3. mongod --auth --port 27017 --dbpath /data/db

启用鉴权后，无密码可以登陆，但是只能执行创建用户操作
...
1. 节点数量建议要5个，2+2+1模式
2. 主数据中心的两个节点要设置高一点的优先级，减少跨中心换主节点
3. 同城双中心之间的网络要保证低延迟和频宽，满足writeConcern：Majority的双中心写需求
4. 使用Retryable Writes and Retryable Reads来保证零下线时间
5. 用户需要自行处理好业务层的双中心切换。


全球集群
中国区用户读写都在本地，
    Primary通过oplog同步到外国的Secondary
    Secondary 通过oplog同步外国的Primary


Zone Sharding 设置步骤
1. 针对每个要分片的数据集合，模型中增加一个区域字段
2. 给集群的每个分片加区域标签
3. 给每个区域指定属于这个区域的分片块防范围（chunk range)

读写场景分析
1. 美国用户浏览商品页面 readConcern ： local   本地读
2. 美国用户下单  writeConcern ： majority    写到本地两个节点
3. 位于总部的业务经历查看销售报表汇总两个分片的数据 readPreference ： nearest 在2个分片的从节点上读取订单数据   本地读
4. 美国用户到中国旅游，访问中国的网站并且下单 writeConcern ： majority   远程写到美国的节点
...
本案例中利用mongo的以下特性：
1. 灵活数据模型，使数据整合更加容易
2. 弹性扩展，使得海量数据+低延迟查询成为可能。

还能利用mongo的反范式的数据模型，使得复杂数据整合成为可能.
...
### 索引执行计划
1. 匹配计划缓存 -> 评估计划性能 -> Y： 6， N 驱逐计划缓存 => 2
2. 生成候选计划
3. 评估候选计划
4. 选择最优计划
5. 创建计划缓存
